...
local moduleName = "cpban"

local messages = {

    ["punishment"] = "{A} Police banned {T} for {V}.",
    ["punishmentHelp"] = "Stops players from becoming police.",

    ["removePunishment"] = "{A} Un policebanned {T}.", 
    ["removePunishmentHelp"] = "Removes police ban.",

    ["punishmentTimeLeft"] = "You're Police banned for {V}."
}
...
sam.command.new( moduleName )
    ...
    :OnExecute( function( ply, targets, length, isSilent )
        ...
        if ( policeTeams[targetTeam] ) then target:changeTeam( defaultTeam, true ) end 
...
hook.Add( "playerCanChangeTeam", "MPunishment:PoliceBan", function( ply, teamID )

    if ( not IsValid( ply ) or not isnumber( teamID ) ) then return end 
    
    if ( not policeTeams[teamID] ) then return end 
...
hook.Add( "OnGamemodeLoaded", "MPunishment:LoadCPTeams", function()

    policeTeams = GAMEMODE.CivilProtection 

    defaultTeam = GAMEMODE.DefaultTeam
